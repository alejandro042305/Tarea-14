# instructions.py
# Set de instrucciones para el computador NB-64 (Tarea 14).
#
# CORRECCIONES APLICADAS (5 bugs resueltos):
#  Bug 1 - Prefijos incorrectos: recalculados para que prefijo = tamaño real.
#  Bug 2 - Conflictos de opcode: opcodes de longitud FIJA por categoria.
#  Bug 3 - RET/IRET crasheaban: logica de pila corregida (convencion estandar).
#  Bug 4 - Loader: corregido en loader.py.
#  Bug 5 - decode_params silencioso: validacion de longitud minima anadida.
#
# ESQUEMA DE OPCODES SIN CONFLICTOS:
#   1 byte  (prefijo 00): clave = 8 bits  [00][id 6b]       params = 0b
#   2 bytes (prefijo 01): clave = 7 bits  [01][id 5b]       params = 9b
#   4 bytes (prefijo 10): clave = 7 bits  [10][id 5b]       params = 25b
#   8 bytes (prefijo 11): clave = 7 bits  [11][id 5b]       params = 57b
#
# DISTRIBUCION DE PARAMS:
#   RR    (2b): [Rn 4b][Rm 4b][pad 1b]
#   R     (2b): [Rn 4b][0000][pad 1b]
#   R_IMM8(4b): [Rn 4b][Imm8 8b][pad 13b]
#   DIR   (8b): [Dir 48b][pad 9b]
#   R_DIR (8b): [Rn 4b][Dir 48b][pad 5b]
#   DIR_R (8b): [Dir 48b][Rm 4b][pad 5b]
...
def _b2i(bits: str, required: int = 0) -> int:
    if required > 0 and len(bits) < required:
        raise ValueError(
            ...
            f"('{bits}'). Verifique el prefijo del opcode en instr_dict."
        )
...
def decode_params(kind: str, params_raw: str) -> Params:
    ...
    if kind == "IMM8":
        _b2i(params_raw, 8)
        p.imm8 = _b2i(params_raw[0:8])
        return p
    raise ValueError(f"Tipo de parametros desconocido: '{kind}'")


# ── Tipo A ────────────────────────────────────────────────────────
def instr_mov_rr(cpu, regs, mem, p):
    regs.set_reg(p.ra, regs.get_reg(p.rb))

def instr_mov_r_dir(cpu, regs, mem, p):
    regs.set_reg(p.ra, _rb(mem, p.addr))

def instr_mov_dir_r(cpu, regs, mem, p):
    _wb(mem, p.addr, regs.get_reg(p.rb))
...
def instr_push(cpu, regs, mem, p):
    if regs.SP <= 0:
        ...
        return
    regs.push_SP(1)
    _wb(mem, regs.SP, regs.get_reg(p.ra))

def instr_pop(cpu, regs, mem, p):
    regs.set_reg(p.ra, _rb(mem, regs.SP))
    regs.pop_SP(1)

def instr_load(cpu, regs, mem, p):
    regs.set_reg(p.ra, _rb(mem, p.addr))

def instr_store(cpu, regs, mem, p):
    _wb(mem, p.addr, regs.get_reg(p.ra))

def instr_xchg(cpu, regs, mem, p):
    a, b = regs.get_reg(p.ra), regs.get_reg(p.rb)
    regs.set_reg(p.ra, b); regs.set_reg(p.rb, a)

def instr_lea(cpu, regs, mem, p):
    regs.set_reg(p.ra, p.addr & 0xFF)
...
def instr_addi(cpu, regs, mem, p):
    a, b = regs.get_reg(p.ra), p.imm8
    res = a + b
    regs.set_reg(p.ra, res); regs.update_flags(res, a, b, "add")
...
def instr_call(cpu, regs, mem, p):
    ...
    for i in range(5, -1, -1):
        if regs.SP <= 0:
            ...
            return
        regs.push_SP(1)
        _wb(mem, regs.SP, (ret >> (8 * i)) & 0xFF)

    regs.PC = p.addr % mem.size


# ── Tipo E ────────────────────────────────────────────────────────
def instr_nop(cpu, regs, mem, p):
    pass
def instr_halt(cpu, regs, mem, p):
    cpu.halted = True
...
def instr_ret(cpu, regs, mem, p):
    ...
    for i in range(5, -1, -1):
        ...
        addr = (addr << 8) | _rb(mem, regs.SP)
        regs.pop_SP(1)

    regs.PC = addr % mem.size
def instr_iret(cpu, regs, mem, p):
    instr_ret(cpu, regs, mem, p)

def instr_int(cpu, regs, mem, p):
    pass  # extension futura


# ── Tabla de instrucciones ────────────────────────────────────────
instr_dict: Dict[str, Tuple[Callable, str]] = {

    # 1 byte (prefijo 00): clave = 8 bits [00][id 6b]
    "00000000": (instr_nop,       "NONE"),
    "00000001": (instr_halt,      "NONE"),
    "00000010": (instr_ret,       "NONE"),
    "00000011": (instr_iret,      "NONE"),

    # 2 bytes (prefijo 01): clave = 7 bits [01][id 5b]
    ...
    # 4 bytes (prefijo 10): clave = 7 bits [10][id 5b]
    "1000000":  (instr_movi,      "R_IMM8"),
    "1000001":  (instr_addi,      "R_IMM8"),
    "1000010":  (instr_int,       "IMM8"),

    # 8 bytes (prefijo 11): clave = 7 bits [11][id 5b]
    "1100000":  (instr_jmp,       "DIR"),
    "1100001":  (instr_jz,        "DIR"),
    "1100010":  (instr_jnz,       "DIR"),
    "1100011":  (instr_jc,        "DIR"),
    "1100100":  (instr_jnc,       "DIR"),
    "1100101":  (instr_call,      "DIR"),
    "1100110":  (instr_mov_r_dir, "R_DIR"),
    "1100111":  (instr_mov_dir_r, "DIR_R"),
    "1101000":  (instr_load,      "R_DIR"),
    "1101001":  (instr_store,     "R_DIR"),
    "1101010":  (instr_lea,       "R_DIR"),
}